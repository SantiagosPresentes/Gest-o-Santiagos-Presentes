...
import {
  ...
  CalendarDays, RefreshCw, Eye, Trophy, Target,
  Banknote, Package, Filter
} from 'lucide-react'

const MESES_NOMES = ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez']
const META_MENSAL = 3000 // ajuste conforme necessário
...
export default function PainelVendedor() {
  ...
  const [todasVendas,   setTodasVendas]   = useState([]) // para ranking
  ...
  // vendas do dia (sem filtro de mês/ano)
  const hoje = new Date(); hoje.setHours(0,0,0,0)
    ...
    <div style={{background:'#f4f6f9',minHeight:'100vh',padding:'0 0 40px 0'}}>
      ...
      <div style={{background:'#fff',borderRadius:'16px',padding:'16px 20px',boxShadow:'0 2px 12px rgba(15,23,42,0.07)',border:'1px solid #eef2f7',margin:'16px 0'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:'12px',alignItems:'end'}}>
          {[
            { label:'Ano',       value:filtroAno,  set:setFiltroAno,  opts:anosDisponiveis.map(a=>({v:String(a),l:String(a)})), placeholder:'Todos anos' },
            { label:'Mês',       value:filtroMes,  set:setFiltroMes,  opts:MESES_NOMES.map((m,i)=>({v:String(i+1),l:m})),       placeholder:'Todos meses' },
            { label:'Situação',  value:filtroSit,  set:setFiltroSit,  opts:Object.entries(SITUACOES).map(([k,v])=>({v:k,l:v.label})), placeholder:'Todas' },
          ].map((f, i) => (
            <div key={i}>
      ...
      <div style={card}>
        ...
        {progMeta >= 100 && <p style={{fontSize:'12px',color:'#10b981',fontWeight:'bold',marginTop:'8px'}}>🎉 Meta atingida!</p>}
        {progMeta < 100 && <p style={{fontSize:'12px',color:'#888',marginTop:'8px'}}>Falta {fmt(META_MENSAL - totalVendido)} para a meta</p>}
      </div>
        ...
        {ranking.map(([nome, total], i) => {
          ...
          return (
            <div key={i} style={{display:'flex',alignItems:'center',gap:'12px',padding:'10px 12px',borderRadius:'10px',marginBottom:'8px',background:isMeu?'#f0faf6':'#f9f9f9',border:isMeu?'1.5px solid #1a6b5a':'1.5px solid transparent'}}>
              <div style={{width:'28px',height:'28px',borderRadius:'50%',background:i===0?'#f7c948':i===1?'#b0bec5':i===2?'#cd7f32':'#e0e0e0',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:'bold',fontSize:'13px',color:i<3?'#333':'#888',flexShrink:0}}>
                {i+1}
              </div>
                ...
                {isOpen && (
                  <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} transition={{duration:0.2}}
                    ...
                    <div style={{padding:'14px 16px'}}>
                      ...
                      {/* Produtos */}
                      <p style={{fontSize:'12px',fontWeight:'700',color:'#888',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:'8px'}}>Produtos</p>
                      {itens.length === 0 ? (
                        <p style={{fontSize:'13px',color:'#aaa'}}>Sem itens registrados</p>
                      ) : itens.map((item, idx) => (
                      ...
                      {/* Pagamento */}
                      <p style={{fontSize:'12px',fontWeight:'700',color:'#888',textTransform:'uppercase',letterSpacing:'0.5px',margin:'14px 0 8px'}}>Pagamento</p>
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'12px'}}>
                        {[
                          { label:'Total',    valor:fmt(venda.valor_total), cor:'#333' },
                          { label:'Recebido', valor:fmt(venda.recebido),    cor:'#10b981' },
                          { label:'Em aberto',valor:fmt(emAberto),          cor:emAberto>0?'#ef4444':'#10b981' },
                      ...
                      <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
                        ...
                        {sit !== 'pago' && (
                          <button onClick={()=>handlePagar(venda)} disabled={isPagando}
                            style={{flex:1,minWidth:'120px',padding:'10px 14px',borderRadius:'10px',border:'none',background:isPagando?'#ccc':'linear-gradient(135deg,#1a6b5a,#10b981)',color:'white',fontWeight:'700',fontSize:'13px',cursor:isPagando?'default':'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'6px'}}>
                            {isPagando?<><RefreshCw size={13} style={{animation:'spin 1s linear infinite'}}/> Salvando...</>:<><CheckCircle2 size={14}/> Marcar como Pago</>}
                          </button>
                        )}
                        {sit === 'pago' && (
                          <div style={{flex:1,minWidth:'120px',padding:'10px 14px',borderRadius:'10px',background:'#e8f5e9',color:'#2e7d32',fontWeight:'700',fontSize:'13px',display:'flex',alignItems:'center',justifyContent:'center',gap:'6px'}}>
