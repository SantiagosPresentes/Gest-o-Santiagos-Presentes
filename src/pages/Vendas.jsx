...
function Vendas() {
  ...
  // Quando o total ou número de parcelas muda, recalcula as parcelas
  useEffect(() => {
    const qtd = parseInt(parcelamento)
    const valorParcela = qtd > 0 ? (total / qtd).toFixed(2) : total.toFixed(2)
  ...
  async function finalizarVenda() {
    ...
    // Monta observação com detalhes do parcelamento
    ...
    // Salva a venda com a primeira parcela como data de pagamento
      ...
      <div className="grid-2" style={{marginTop:'16px'}}>
        ...
        <div style={{background:'white', padding:'24px', borderRadius:'12px', boxShadow:'0 2px 8px rgba(0,0,0,0.08)'}}>
          ...
          <div style={{display:'flex', gap:'8px', marginBottom:'16px'}}>
            ...
            />
            <button onClick={buscarProduto} style={{background:'#1a6b5a', color:'white', border:'none', padding:'10px 16px', borderRadius:'6px', cursor:'pointer', fontWeight:'bold'}}>+</button>
          </div>
          ...
          {itens.map(item => (
            <div key={item.produto_id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px', background:'#f8f8f8', borderRadius:'8px', marginBottom:'8px'}}>
              <div>
                ...
                <small style={{color:'#666'}}>{item.categoria} | R$ {item.valor_unitario.toFixed(2)}</small>
              </div>
              <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
                ...
                <button onClick={() => alterarQuantidade(item.produto_id, item.quantidade + 1)} style={{background:'#ddd', border:'none', padding:'4px 10px', borderRadius:'4px', cursor:'pointer'}}>+</button>
                <strong style={{color:'#1a6b5a'}}>R$ {item.subtotal.toFixed(2)}</strong>
              </div>
            </div>
          ))}
        ...
        <div style={{background:'white', padding:'24px', borderRadius:'12px', boxShadow:'0 2px 8px rgba(0,0,0,0.08)'}}>
          ...
          <div style={{marginBottom:'16px', background:'#f8f8f8', padding:'16px', borderRadius:'8px', border:'1px solid #eee'}}>
            ...
            {parcelas.map((p, i) => (
              <div key={i} style={{display:'flex', gap:'8px', alignItems:'center', marginTop:'10px'}}>
                <span style={{fontSize:'13px', color:'#666', whiteSpace:'nowrap', minWidth:'60px'}}>
