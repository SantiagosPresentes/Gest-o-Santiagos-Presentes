    ...
    <div>
      ...
      {pagamentoVenda && (
        <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000}}>
          <div style={{background:'white', padding:'32px', borderRadius:'16px', width:'400px', boxShadow:'0 8px 32px rgba(0,0,0,0.3)'}}>
            <h3 style={{marginBottom:'8px'}}>Registrar Pagamento</h3>
            ...
            <label style={{fontSize:'14px'}}>Valor recebido agora (R$)</label><br/>
            <input
              type="number"
              value={valorPago}
            ...
            <div style={{display:'flex', gap:'8px'}}>
              <button onClick={registrarPagamento} style={{flex:1, background:'#1a6b5a', color:'white', border:'none', padding:'12px', borderRadius:'8px', cursor:'pointer', fontSize:'15px', fontWeight:'bold'}}>
                Confirmar Pagamento
              </button>
      ...
      {/* Barra de filtros - todos na mesma linha */}
      <div style={{background:'white', padding:'16px 24px', borderRadius:'12px', marginTop:'16px', boxShadow:'0 2px 8px rgba(0,0,0,0.1)', display:'flex', gap:'16px', alignItems:'center', flexWrap:'wrap'}}>
        <div>
          <label style={{fontSize:'13px', color:'#666'}}>Situação</label><br/>
        ...
        <div style={{marginTop:'18px'}}>
          <button onClick={() => { setFiltroSituacao(''); setFiltroDataInicio(''); setFiltroDataFim(''); setFiltroCliente('') }} style={{background:'#eee', border:'none', padding:'8px 16px', borderRadius:'6px', cursor:'pointer'}}>
            Limpar Filtros
          </button>
        </div>
        <div style={{marginTop:'18px', marginLeft:'auto', color:'#666', fontSize:'14px'}}>
          {vendasFiltradas.length} venda(s) encontrada(s)
        </div>
      </div>

      {/* Mensagem de sucesso fora do modal */}
      ...
      <div style={{background:'white', borderRadius:'12px', marginTop:'16px', boxShadow:'0 2px 8px rgba(0,0,0,0.1)', overflow:'hidden'}}>
        <table style={{width:'100%', borderCollapse:'collapse'}}>
          <thead>
            <tr style={{background:'#1a1a2e', color:'white'}}>
              ...
              <th style={{padding:'14px 16px', textAlign:'left'}}>Produtos</th>
              <th style={{padding:'14px 16px', textAlign:'right'}}>Total</th>
              <th style={{padding:'14px 16px', textAlign:'right'}}>Recebido</th>
              <th style={{padding:'14px 16px', textAlign:'right'}}>Falta</th>
              <th style={{padding:'14px 16px', textAlign:'center'}}>Vencimento</th>
              <th style={{padding:'14px 16px', textAlign:'center'}}>Situação</th>
              <th style={{padding:'14px 16px', textAlign:'left'}}>Observação</th>
              <th style={{padding:'14px 16px', textAlign:'center'}}>Ação</th>
            </tr>
          </thead>
          <tbody>
            {vendasFiltradas.map((venda, i) => {
              ...
              return (
                <>
                  <tr key={venda.id} style={{background: i % 2 === 0 ? '#fff' : '#f9f9f9', borderBottom:'1px solid #eee'}}>
                    <td style={{padding:'12px 16px'}}>
                      <strong>{venda.clientes?.nome}</strong><br/>
                      <small style={{color:'#888'}}>{venda.clientes?.telefone}</small>
                    </td>
                    <td style={{padding:'12px 16px'}}>
                      <span onClick={() => setVendaExpandida(vendaExpandida === venda.id ? null : venda.id)} style={{cursor:'pointer', color:'#1a1a2e', textDecoration:'underline', fontSize:'13px'}}>
                        {venda.itens?.length} produto(s) — ver detalhes
                      </span>
                    </td>
                    <td style={{padding:'12px 16px', textAlign:'right'}}>
                      <strong>R$ {parseFloat(venda.valor_total).toFixed(2)}</strong>
                    </td>
                    <td style={{padding:'12px 16px', textAlign:'right', color:'green'}}>
                      R$ {parseFloat(venda.recebido || 0).toFixed(2)}
                    </td>
                    <td style={{padding:'12px 16px', textAlign:'right', color:'red'}}>
                      R$ {(parseFloat(venda.valor_total) - parseFloat(venda.recebido || 0)).toFixed(2)}
                    </td>
                    ...
                    <td style={{padding:'12px 16px', textAlign:'center'}}>
                      <span style={{...corSituacao(venda.situacao_real), padding:'4px 12px', borderRadius:'20px', fontSize:'12px', fontWeight:'bold'}}>
                        {venda.situacao_real}
                      </span>
                    </td>
                    ...
                    <td style={{padding:'12px 16px', textAlign:'center'}}>
                      {venda.situacao_real !== 'Pago' && (
                        <button onClick={() => { setPagamentoVenda(venda); setMensagem('') }} style={{background:'#1a6b5a', color:'white', border:'none', padding:'6px 12px', borderRadius:'6px', cursor:'pointer', fontSize:'12px', fontWeight:'bold'}}>
                          💰 Pagar
                        </button>
                      )}
                      {venda.situacao_real === 'Pago' && (
                        <span style={{color:'green', fontSize:'13px'}}>✅ Quitado</span>
                      )}
                    </td>
                  </tr>
                  {vendaExpandida === venda.id && (
                    <tr key={venda.id + '_detalhes'}>
                      <td colSpan="9" style={{padding:'0 16px 12px 32px', background:'#f0f4ff'}}>
                        <strong style={{fontSize:'13px'}}>Produtos desta venda:</strong>
                        <div style={{marginTop:'8px', display:'flex', flexWrap:'wrap', gap:'8px'}}>
                          {venda.itens?.map(item => (
                            <span key={item.id} style={{background:'white', border:'1px solid #ddd', padding:'6px 12px', borderRadius:'8px', fontSize:'13px'}}>
                        ...
                        {devs.length > 0 && (
                          <div style={{marginTop:'12px'}}>
                            ...
                            <div style={{marginTop:'6px', display:'flex', flexWrap:'wrap', gap:'8px'}}>
                              {devs.map(d => (
                                <span key={d.id} style={{background:'#fff0f3', border:'1px solid #e94560', padding:'6px 12px', borderRadius:'8px', fontSize:'13px'}}>
        ...
        {vendasFiltradas.length === 0 && (
          <p style={{textAlign:'center', padding:'32px', color:'#aaa'}}>Nenhuma venda encontrada</p>
        )}
      </div>
    </div>
  )
