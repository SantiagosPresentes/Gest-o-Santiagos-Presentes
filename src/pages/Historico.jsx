...
function Historico() {
  ...
  async function carregarTudo() {
    ...
    try {
      ...
      const { data: vendasData, error: erroVendas } = await supabase
        .from('vendas')
        .select('id, cliente_id, valor_total, recebido, desconto, data_venda, data_para_pagar, situacao, observacao, vendedor_nome, clientes(nome, telefone)')
      ...
      // Busca itens de todas as vendas de uma vez
      const idsVendas = vendasData.map(v => v.id)
      ...
      if (erroItens) console.warn('Erro ao buscar itens:', erroItens.message)

      // Busca devoluções
      const { data: devolucoesData } = await supabase
        .from('devolucoes')
        .select('id, venda_id, quantidade, valor_total, motivo, clientes(nome), produtos(nome)')
        .order('criado_em', { ascending: false })

      if (devolucoesData) setDevolucoes(devolucoesData)

      // Monta vendas com itens
      const itensPorVenda = {}
      if (todosItens) {
      ...
      const vendasMontadas = vendasData.map(venda => ({
        ...venda,
  ...
  const vendasFiltradas = vendas.filter(v => {
    ...
    const dataBase = v.data_venda ? v.data_venda.substring(0, 10) : ''
    if (filtroDataInicio && dataBase < filtroDataInicio) return false
    if (filtroDataFim && dataBase > filtroDataFim) return false
    return true
  })

  function corSituacao(situacao) {
    if (situacao === 'Pago') return { background: '#e8f5e9', color: '#2e7d32' }
    if (situacao === 'Atrasado') return { background: '#ffebee', color: '#c62828' }
  ...
  function fmtData(str) {
    if (!str) return '—'
    return new Date(str.includes('T') ? str : str + 'T12:00:00').toLocaleDateString('pt-BR')
  }
    ...
    <div style={{ background: '#f4f6f9', minHeight: '100vh', paddingBottom: '40px' }}>
          ...
          <div style={{ background: 'white', borderRadius: '16px', width: '100%', maxWidth: '440px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
            <div ref={comprovanteRef} style={{ padding: '24px' }}>
              <div style={{ textAlign: 'center', marginBottom: '12px' }}>
                ...
                <p style={{ color: '#999', fontSize: '12px' }}>{fmtData(comprovanteVenda.data_venda || comprovanteVenda.data_para_pagar)}</p>
              </div>
              ...
              {comprovanteVenda.itens?.map((item, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '6px' }}>
                  ...
                  <span style={{ color: '#666', marginRight: '8px' }}>{item.quantidade}x {fmtMoeda(item.valor_unitario)}</span>
                  <strong>{fmtMoeda(item.quantidade * parseFloat(item.valor_unitario))}</strong>
                </div>
              ))}
              ...
              {parseFloat(comprovanteVenda.desconto || 0) > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', color: '#2e7d32', marginBottom: '4px' }}>
                  <span>Desconto</span><span>- {fmtMoeda(comprovanteVenda.desconto)}</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '18px', fontWeight: 'bold', color: '#1a6b5a', marginBottom: '8px' }}>
                <span>TOTAL</span><span>{fmtMoeda(comprovanteVenda.valor_total)}</span>
              </div>
              {(() => {
                ...
                return parcelas ? (
                  ...
                  <div style={{ fontSize: '13px', color: '#555', marginBottom: '8px' }}>
                    <strong>Vencimento:</strong> {fmtData(comprovanteVenda.data_para_pagar)}
                  </div>
                )
              })()}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '16px' }}>
          <div style={{ background: 'white', padding: '32px', borderRadius: '16px', width: '100%', maxWidth: '400px', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
            ...
            <p style={{ color: '#666', marginBottom: '16px', fontSize: '14px' }}>
              ...
              Total: <strong>{fmtMoeda(pagamentoVenda.valor_total)}</strong><br />
              Já recebido: <strong style={{ color: 'green' }}>{fmtMoeda(pagamentoVenda.recebido)}</strong><br />
              Falta: <strong style={{ color: 'red' }}>{fmtMoeda(parseFloat(pagamentoVenda.valor_total) - parseFloat(pagamentoVenda.recebido || 0))}</strong>
            </p>
      ...
      <div style={{ background: 'white', padding: '16px', borderRadius: '12px', marginTop: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', display: 'flex', gap: '12px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={{ background: '#e94560', border: 'none', padding: '8px 14px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: 'white' }}
        >
          🧹 Limpar
        </button>
        <button
          onClick={carregarTudo}
          style={{ background: '#1a6b5a', border: 'none', padding: '8px 14px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: 'white' }}
        >
          🔄 Atualizar
        </button>
        <div style={{ marginLeft: 'auto', color: '#666', fontSize: '13px', whiteSpace: 'nowrap' }}>
          {loading ? '⏳ Carregando...' : `${vendasFiltradas.length} venda(s)`}
        </div>
      </div>
      ...
      {/* ── Erro ── */}
      {erro && (
        <div style={{ background: '#ffebee', border: '1px solid #e53935', borderRadius: '8px', padding: '12px 16px', marginTop: '12px', color: '#c62828' }}>
          ⚠️ {erro}
          <button onClick={carregarTudo} style={{ marginLeft: '12px', background: '#c62828', color: 'white', border: 'none', padding: '4px 10px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>
            Tentar novamente
          </button>
        </div>
      )}
      ...
      {/* ── Loading ── */}
      {loading && (
        <div style={{ background: 'white', borderRadius: '12px', padding: '48px', textAlign: 'center', marginTop: '16px', color: '#888' }}>
          ⏳ Carregando vendas...
        </div>
      )}
        ...
        <div className="tabela-wrapper" style={{ marginTop: '16px' }}>
                ...
                return (
                  <>
                    <tr key={venda.id} style={{ background: i % 2 === 0 ? '#fff' : '#f9f9f9' }}>
                      ...
                      <td style={{ textAlign: 'left', fontSize: '13px' }}>{venda.vendedor_nome || '—'}</td>
                      <td style={{ textAlign: 'left' }}>
                        <span
                      ...
                      <td style={{ textAlign: 'right' }}><strong>{fmtMoeda(venda.valor_total)}</strong></td>
                      <td style={{ textAlign: 'right', color: parseFloat(venda.desconto || 0) > 0 ? '#e65100' : '#aaa', fontSize: '13px' }}>
                        {parseFloat(venda.desconto || 0) > 0 ? `- ${fmtMoeda(venda.desconto)}` : '—'}
                      </td>
                      <td style={{ textAlign: 'right', color: 'green' }}>{fmtMoeda(venda.recebido)}</td>
                      <td style={{ textAlign: 'right', color: falta > 0 ? 'red' : 'green' }}>{fmtMoeda(falta)}</td>
                      <td style={{ textAlign: 'center', whiteSpace: 'nowrap' }}>{fmtData(venda.data_para_pagar)}</td>
                          ...
                          <div style={{ display: 'flex', gap: '16px', marginBottom: '10px', fontSize: '13px', flexWrap: 'wrap' }}>
                            ...
                            {parseFloat(venda.desconto || 0) > 0 && (
                              <span style={{ color: '#e65100' }}>
                                <strong>Desconto aplicado:</strong> - {fmtMoeda(venda.desconto)}
                              </span>
                            )}
                          </div>
                          ...
                          <div style={{ marginTop: '8px', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                            {venda.itens?.map(item => (
                              <span key={item.id} style={{ background: 'white', border: '1px solid #ddd', padding: '6px 12px', borderRadius: '8px', fontSize: '12px' }}>
                                {item.produtos?.nome} — {item.quantidade}x — {fmtMoeda(item.valor_unitario)}
                              </span>
                            ))}
                          </div>
                          {devs.length > 0 && (
                              ...
                              <div style={{ marginTop: '6px', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                                {devs.map(d => (
                                  <span key={d.id} style={{ background: '#fff0f3', border: '1px solid #e94560', padding: '6px 12px', borderRadius: '8px', fontSize: '12px' }}>
                                    {d.produtos?.nome} — {d.quantidade}x — {fmtMoeda(d.valor_total)} — {d.motivo}
                                  </span>
                                ))}
                              </div>
                            </div>
          ...
          {vendasFiltradas.length === 0 && !loading && (
            <p style={{ textAlign: 'center', padding: '32px', color: '#aaa', background: 'white' }}>
              {vendas.length === 0 ? 'Nenhuma venda encontrada no banco de dados' : 'Nenhuma venda corresponde aos filtros'}
            </p>
          )}
        </div>
      )}
